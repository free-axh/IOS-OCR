...
-(UITableViewCell *)nonFreightCarContent:(UITableViewCell *)cell indexPathRow:(NSInteger)index
{
  if (index == 0) {
    ...
    UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, self.view.frame.size.width - 30, 30)];
...
- (void)uploadPositiveEvent
{
  ...
  if ([[OCRSingleton sharedSingleton].carType isEqualToString:@"1"]) {
      image = [[self.vehicleLicensePositiveInfoData objectAtIndex:1] objectForKey:@"value"];
  } else {
     image = [[self.nonFreightCarData objectAtIndex:2] objectForKey:@"value"];
  }
...
{
  UIImage *image = [[self.vehicleLicenseReverseInfoData objectAtIndex:1] objectForKey:@"value"];
  NSDictionary *imageData = @{
                              @"decodeImage": image,
                              };
  [[OCRServices shardService] uploadImage:imageData
